...
In venture capital markets (innovation financing included), syndication serves as a coordination mechanism used by Venture Capitalists (VCs) to operate in complex networks where investment decisions are embedded within social structures, geographic clusters, and institutional relationships.
...
This study aims to apply Network Science theories and methods to analyse these coordination patterns in the syndication network of early and late stage investors operating in the U.S. market, revealing how investors organise into distinct network communities with different structural properties and organisational patterns that may influence capital allocation efficiency, deal flow, and market access.

Special attention is given to the fact that a community of prominent VCs in Silicon Valley demonstrates nested hierarchical organisation patterns that develop progressively over time, with nestedness values beginning to increase after 2012 and reaching statistical significance around 2019, while other communities maintain more random, hard-to-define partnership structures. The specific factors underlying this progressive structural evolution remain unexplored in this work, presenting opportunities for future research into the mechanisms driving hierarchical organisation in innovation networks \cite{Mariani2019, Barabasi1999, Scheffer2009}.

The following sections examine the theoretical foundations for syndication behaviour, drawing from literature in innovation financing, social embeddedness, network theory, and ecological systems to build a comprehensive framework for understanding syndication investment network organisation. 

Nevertheless, implications for market efficiency, geographic clustering effects, and the role of innovation hubs in shaping investment ecosystems are discussed, as well as future research opportunities are presented.

\subsection{Syndication in Innovation Financing}

Theoretical foundations for syndication behaviour come from literature on screening and information sharing among venture capitalists \cite{Casamatta2007}. Syndication became relevant in that context because, in markets characterised by incomplete information (e.g. innovation financing), the decision to syndicate and the choice of syndication partners are aspects that influence both investment outcomes and network position \cite{Casamatta2007}.
...
In this context, considering that screening innovative ventures is frequently a multi-stage process (venture capitalists define formal pipelines which startups and often their founders need to pass through), syndication plays an important role in reducing the effort and possibly the number of screening stages, with the cost of disclosing information, as investors are essentially competitors.
...
Economic sociologists have demonstrated that economic action is fundamentally embedded in ongoing social networks \cite{Granovetter1985}. For instance, entrepreneurs and venture capitalists develop recurring partnerships built on trust and reputation rather than purely transactional relationships \cite{Uzzi1997}. This embeddedness perspective reveals that syndication patterns emerge from social structures that extend beyond individual deal characteristics.

Central to this framework is Burt's concept of structural holes \cite{Burt1992}, which shows how investors who bridge otherwise disconnected groups gain access to unique information and control over resource flows (advantages that directly complement the screening benefits discussed earlier). In that same manner, geographic clustering in innovation hubs such as Silicon Valley facilitates the repeated face-to-face interactions that strengthen these social ties, creating the relational foundation upon which systematic syndication patterns can emerge.

% In venture capital markets, this embeddedness manifests in several interconnected ways that directly influence network organization. First, investment decisions are embedded within social networks where venture capitalists develop recurring partnerships based on trust, shared investment philosophies, and successful collaboration histories \cite{Granovetter1985}. These relationships create persistent patterns of co-investment that go beyond individual deal characteristics, forming the basis for community structure within investor networks.
...
In the following sub-sections we explore concepts spanning all of those fields to set the theoretical basis for our reasoning on the syndication networks construction and analysis for further chapters.
...
A unipartite network has a single set of nodes, and any two nodes may be linked (e.g., people connected to people). On the other hand, a bipartite network has two disjoint sets of nodes that play different roles, and links are allowed only between the sets (e.g., plants–pollinators \cite{Dormann2009} or early–late investors), never within a set. In summary, bipartite models are useful when role separation matters. Figure~\ref{fig:unipartite_vs_bipartite} provides an illustration of both types of networks.
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{../figures/diagrams/unipartite_vs_bipartite_networks.png}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\textwidth]{../figures/diagrams/classical_syndication_net.png}
...
\subsubsection{Bipartite Early-Late Syndication Model}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\textwidth]{../figures/diagrams/bipartite_projections.png}
...
This projection mechanism provides a bridge between our role-aware bipartite model and classical syndication networks. When we project our Early-Late model, we recover something similar to classical investor-investor networks, even though this work does not explicitly model and explore this dynamic.

In summary, our bipartite approach enables the study of hierarchical patterns like nestedness (something obscured in classical models) while maintaining compatibility with existing venture capital research.
...
In our case, we detect communities by maximizing \emph{modularity}, which scores a partition by comparing the observed number of within-group edges to what a degree-preserving random network would produce. In its common form,
...
Lastly, the concept of power-law degree distributions characterize the heterogeneous heavy-tailed connectivity patterns in venture capital networks \cite{Bygrave1987}, which basically describes systems where a small number of investors (hubs) maintain extensive partnership networks while most participants have relatively few connections.
...
Ecology theory offers additional insight into these organisation of complex networks. In special, studies of mutualistic pollinator-plant networks have shown that interactions often form nested structures in which specialist pollinators (those that visit few plant species) typically interact with subsets of the same plants as generalist pollinators (those that visit many species) \cite{Bascompte2003,Mariani2019}. 
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\textwidth]{../figures/diagrams/unipartite_bipartite_nested_net.png}
...
The remainder of this paper is organized as follows. The \textit{Methodology} section establishes our analytical framework by detailing the Crunchbase data preprocessing procedures, the construction of bipartite early-late stage investor networks, and the implementation of community detection and nestedness measurement techniques. The \textit{Results} section first provides a comprehensive dataset characterization including syndication trends and investment activity patterns, then presents the identification and analysis of investor communities through modularity optimization, examines their geographic distributions and funding behaviors, and finally quantifies nestedness patterns using NODF metrics and statistical significance testing. The \textit{Discussion} section interprets these empirical findings by exploring the implications of hierarchical organization for investment efficiency, analyzing the temporal dynamics of progressive nestedness development, and examining the relationship between geographic clustering and network topology. Finally, the \textit{Conclusion} synthesizes our key contributions, acknowledges study limitations, and outlines directions for future research into network-driven innovation financing mechanisms.
