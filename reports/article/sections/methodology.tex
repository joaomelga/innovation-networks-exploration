...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{../figures/diagrams/vc_investment_bipartite_pipeline.png}
...
To determine whether observed nestedness values differ significantly from what would be expected by chance, we employ a null model approach using the Curveball algorithm \cite{Strona2014}. Curveball generates randomised matrices that preserve the degree sequence of both node sets while randomising the connection patterns.  
...
For each community, we compute the mean ($\mu_{\mathrm{null}}$) and standard deviation ($\sigma_{\mathrm{null}}$) of the null NODF values. We then calculate a standardised Z-score to quantify how many standard deviations the observed nestedness differs from the null expectation:
