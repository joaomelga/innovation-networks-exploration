...
\begin{figure}[htbp]
    ...
    \caption{Left, a unipartite network—one set of nodes (white) where edges can connect any pair. Right, a bipartite network—two disjoint sets (green vs. blue) with edges only across sets and none within a set. Use bipartite when nodes play different roles (e.g., plants–pollinators or early–late investors); the familiar investor–investor view comes from projecting a bipartite network onto one side.}
